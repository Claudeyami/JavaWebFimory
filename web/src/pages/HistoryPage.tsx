...
const HistoryPage: React.FC = () => {
  ...
  const deleteMovieHistory = async (movieId: number) => {
    if (!email) return;
    try {
      ...
      if (res.ok) {
        setMovieItems((prev) => prev.filter((i) => i.MovieID !== movieId));
      }
    } catch {}
  };

  const deleteSeriesHistory = async (seriesId: number) => {
    if (!email) return;
    try {
      ...
      const res = await fetch(url.toString(), { method: 'DELETE' });
      if (res.ok) {
        setSeriesItems((prev) => prev.filter((i) => i.SeriesID !== seriesId));
      }
    } catch {}
  };
          ...
          {movieItems.map(item => (
            <div key={item.HistoryID} className="p-3 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => deleteMovieHistory(item.MovieID)} className="text-sm text-red-600 dark:text-red-400 hover:underline">Xóa</button>
              </div>
            </div>
          ))}
          {movieItems.length === 0 && (
          ...
          {seriesItems.map(item => (
            <div key={item.HistoryID} className="p-3 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={() => deleteSeriesHistory(item.SeriesID)} 
