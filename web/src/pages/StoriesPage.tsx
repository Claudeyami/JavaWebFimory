              ...
              .map(async (s: any) => {
                ...
                try {
                  ...
                  if (categoriesResponse.ok) {
                    ...
                    const categoriesData = Array.isArray(categoriesPayload) ? categoriesPayload : (categoriesPayload?.data ?? []);
                ...
                try {
                  ...
                  if (chaptersResponse.ok) {
                    ...
                    const chaptersData = Array.isArray(chaptersPayload) ? chaptersPayload : (chaptersPayload?.data ?? []);
                    storyChapters = chaptersData.map((ch: any) => ({
                  ...
                  console.error(`Error loading chapters for story ${s.SeriesID}:`, error);
                }

                const latestChapterNumber = storyChapters.length > 0
                  ? Math.max(...storyChapters.map((ch: any) => Number(ch.chapterNumber || 0)))
                  : Number(s.LatestChapterNumber || 0);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Truyện</h1>
      </div>
      ...
      {stories.length === 0 ? (
        <Card>
          <div className="p-12 text-center">
            ...
            <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2">Chưa có truyện nào</h3>
            <p className="text-gray-500 dark:text-gray-400">Hiện tại chưa có truyện nào được xuất bản.</p>
          </div>
        </Card>
      ) : (
